...
const ATTRIBUTES: Array<{ field: keyof CreateCharacterData; label: string }> = [
  { field: 'strength', label: 'FOR' },
  { field: 'dexterity', label: 'DES' },
  { field: 'constitution', label: 'CON' },
  { field: 'intelligence', label: 'INT' },
  { field: 'wisdom', label: 'SAB' },
  { field: 'charisma', label: 'CAR' },
]
...
export function Characters() {
  ...
  const [modal, setModal] = useState<'create' | 'edit' | 'ai' | 'view' | null>(null)
  const [selectedCharacter, setSelectedCharacter] = useState<Character | null>(null)
  const [form, setForm] = useState<CreateCharacterData>(emptyCharacter)
  ...
  async function handleDelete(id: string) {
    if (!user) return
    const confirmed = window.confirm('Deseja realmente excluir este personagem?')
    if (!confirmed) return
  ...
  async function handleSave(event: React.FormEvent) {
    ...
    try {
      ...
      if (modal === 'edit' && selectedCharacter) {
        const updated = await updateCharacter(selectedCharacter.id, form, token)
        setCharacters(current =>
          current.map(char => (char.id === updated.id ? updated : char))
        )
      } else {
        const created = await createCharacter(form, token)
      ...
      console.error(err)
      setError('Não foi possível salvar o personagem.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const filteredCharacters = characters.filter(character => {
    const text = `${character.name} ${character.race} ${character.className}`
  ...
  return (
    <div className="characters-page">
      <header className="characters-header">
        <div>
          <span className="page-label">SUA COLEÇÃO</span>
          <h1>Meus Personagens</h1>
          <p>Crie, organize e prepare seus personagens para qualquer aventura.</p>
        </div>

        <div className="characters-header-actions">
          <button className="secondary-button" onClick={() => setModal('ai')}>
            <Sparkles size={16} />
            Criar com IA
          </button>
      ...
      {loading ? (
        <div className="characters-loading">Carregando personagens...</div>
        ...
        <div className="characters-grid">
          ...
          <button className="character-add-card" onClick={openCreate}>
            <Plus size={30} />
      ...
      {(modal === 'create' || modal === 'edit') && (
        <CharacterFormModal
          title={modal === 'edit' ? 'Editar personagem' : 'Novo personagem'}
          form={form}
          setForm={setForm}
      ...
      {modal === 'ai' && (
        <AIModal
          ...
          onGenerate={async prompt => {
            ...
            try {
              const token = await user.getIdToken()
              const generated = await generateCharacter(prompt, token)
              setForm({ ...emptyCharacter, ...generated })
              setModal('create')
            } catch {
...
interface CharacterFormModalProps {
  ...
  setForm: React.Dispatch<React.SetStateAction<CreateCharacterData>>
  onClose: () => void
...
function CharacterFormModal({
  ...
  function update<K extends keyof CreateCharacterData>(field: K, value: CreateCharacterData[K]) {
          ...
          <section>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={e => update('abilities', e.target.value.split('\n').filter(Boolean))}
              />
            </label>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={e => update('spells', e.target.value.split('\n').filter(Boolean))}
              />
            </label>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={e => update('inventory', e.target.value.split('\n').filter(Boolean))}
              />
            </label>
          </section>
